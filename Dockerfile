...
# Publish API as self-contained (justified, since the CUDA image doesn't have .NET)
RUN dotnet publish "src/InstantAIGate.API/InstantAIGate.API.csproj" \
    -c Release -o /app/publish/api -r linux-x64 --self-contained true 
...
EXPOSE 80
# Copy ALL native libraries PRESERVING THE BACKENDS STRUCTURE
COPY .runtimes/linux-x64/ /app/runtimes/linux-x64/
...
# and not in /app, where old/broken files from previous runs might remain
